...
Adapt the loader to your dataset by editing its **config block** (`SOURCE_ROOT`, `PARTITIONS` — a list, `["."]` for a flat layout — `ASSET_TABLE`, `FILE_TYPES`, `FILE_DATASET_TYPE`, `LABEL_MANIFEST`, the `rename_file` hook) and filling the `stage_source()` stub in `scripts/stage_source_template.py` (its docstring states the layout contract). The register/upload phases are generic and need no edit for a standard **flat** layout (source files → asset rows → a per-asset feature). The sibling templates: `scripts/register_phase_template.py` (Exec 1) and `scripts/upload_phase_template.py` (Exec 2).

> **Parent-entity grouping (optional).** If each asset belongs under a **parent entity** — `Subject → Observation → Image`, or a **multimodal** set grouping SLO/OCT/VF Images under one `Observation` — the loader has two optional seams (default off, so a flat dataset is unchanged): `ASSET_METADATA` sets extra columns on each asset row (a parent FK, a modality tag) via `exe.asset_file_path(metadata=...)` (deriva-ml ≥ 1.54.0), and `CREATE_PARENTS` creates/looks up the parent rows before the upload loop. The parent **table** + the asset's FK column are created in `setup_domain_model_template` step 7 (a commented block to uncomment). This deliberately covers one parent (and a grandparent), not a generic entity graph — keeping the template copy-me, not a framework.
