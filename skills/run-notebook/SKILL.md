...
Two override pitfalls bite agents repeatedly: there is **no `--config*` flag** (`--config`/`--config-name`/`--config-file` cannot swap the config name baked into the notebook's first cell — use positional `assets=`/`deriva_ml=` overrides instead), and **custom scalar fields do not ride along on group overrides** (overriding `assets=...` swaps the group but silently keeps the dataclass default for any sibling config's scalar fields like `confidence_threshold`). See `references/workflow.md` → "Override Pitfalls (the `--config*` trap and scalar fields)" for the full explanation, the wrong-vs-right invocations, and the two patterns that fix the scalar-field case.

### Targeting a different host or catalog

The CLI's `--host`/`--catalog` flags **do not redirect the notebook's catalog connection** — they go to papermill and `run_notebook()` never reads them. To target a different host/catalog, register a named `DerivaMLConfig` (group `deriva_ml`) and select it with a positional override: `deriva_ml=localhost_1407`. See `references/workflow.md` → "Targeting a Different Host or Catalog" for the full recipe (registering the config in `src/configs/dev/` plus the override invocation).

## Concrete Example: ROC Analysis Notebook

A complete worked example from the model template — comparing model predictions across experiments, with the full config module, init cell, downloaded-asset access, output saving, and the run invocations — is in `references/workflow.md` → "Worked Example: ROC Analysis Notebook".
