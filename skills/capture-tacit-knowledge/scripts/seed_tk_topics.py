...
def render_empty_index_md() -> str:
    ...
    return "\n".join(
        [
            ...
            "## Rows",
            "",
            "Each row points at an entry in `tacit-knowledge.md`. `tk-NNN` is a click-through",
            "link to the entry's anchor; `relationships` mirrors the entry's `Supported by:` /",
            "`Supersedes:` edges (the authoritative DAG lives in the entries); `aliases` mirrors",
            "the topic-CV synonyms of the row's keywords.",
            "",
