...
| `tacit-knowledge.md` (root) | present, tracked in git, opens with `type: Log` frontmatter, not gitignored | The append-only tacit-knowledge Log, scaffolded by `seed_tk_topics.py` (see `/deriva-ml:setup-derivaml-project` → "Seed the tacit-knowledge system"); captures decisions over time |
...
| `docs/tacit-knowledge/topics.md` | present, `type: Vocabulary` (controlled-term list), reviewed (not just the raw seed) | The topic CV — human-reviewed, not left as the raw seed output |
| `docs/tacit-knowledge/retrieval-catalog.md` | present, `type: RetrievalCatalog`, has `covers_through` | Derived retrieval catalog — do not hand-edit; rebuilt by `capture-tacit-knowledge` |
| `docs/domain/index.md` | present, **no frontmatter** (OKF `index.md` convention), body lists the `Concept` docs | Domain-background bundle root — a frontmatter-free OKF index over the `type: Concept` docs |
| `.gitattributes` | present, has `merge=union` for the Log, CV, and index | Merge drivers so concurrent tacit-knowledge writes don't conflict |

**Common gaps:** missing `tacit-knowledge.md` (suggest creating an empty file with a one-line header so `capture-tacit-knowledge` has somewhere to write); missing `Experiments.md` (offer to scaffold one from `src/configs/experiments.py` and `src/configs/multiruns.py`); missing `tests/` (offer to create the directory with a placeholder `__init__.py`). Do **not** flag a missing `docs/design/` on a new or early project — design docs are authored on demand by `design-experiment`, so their absence is normal until the user designs their first experiment/dataset/feature/model; report it as "not yet used" rather than a gap. Also: a design doc's "Upstream designs" link pointing at a not-yet-authored design is **not** a defect — OKF tolerates broken links (planned-but-unauthored knowledge), so do not flag a dangling inter-spec link. Missing any of the five tacit-knowledge artifacts (`tacit-knowledge.md`, `docs/tacit-knowledge/topics.md`, `docs/tacit-knowledge/retrieval-catalog.md`, `docs/domain/index.md`, `.gitattributes` merge drivers) — suggest running `seed_tk_topics.py` per `/deriva-ml:setup-derivaml-project`. A `docs/tacit-knowledge/topics.md` that still matches the raw seed output verbatim (no project-specific terms added) is a **partial**, not a pass — flag it for human review per the `term-naming-strategy` discipline.
