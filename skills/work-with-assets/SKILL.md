...
2. **Tag / untag an asset with a type:** `deriva_ml_update_asset(hostname, catalog_id, asset_rid, asset_types=[<desired final list>])`. `asset_types` is **set-style** — pass the full list you want the asset to end up with; the tool diffs against the current tags and adds/removes accordingly. Pass `[]` to clear all caller-supplied tags. Going through this tool (rather than raw `update_entities` on the asset row) keeps the Asset abstraction's provenance, version, and audit machinery intact — see the inheritance-with-override rule in `/deriva-ml:deriva-ml-context`. See `references/workflow.md` for the full recipe.
