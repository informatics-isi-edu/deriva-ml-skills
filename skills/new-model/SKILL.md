...
> should have a `docs/design/model/<slug>.md` at Approved (see
...
> **Before creating a new Workflow, search first.** The `/deriva:semantic-awareness` skill *(deriva-skills, auto-fires plugin-wide)* carries the find-before-you-create discipline — search the catalog for an existing Workflow whose name, type, or description overlaps with what you're about to register, and reuse it if it does. The same discipline applies to ML entities (Workflows, Datasets, Features) as to generic catalog entities. The same skill also covers the EAV-vs-wide-table dual extreme to steer away from when designing supporting tables — see it for the worked treatment.
