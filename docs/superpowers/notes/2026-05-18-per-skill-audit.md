...
| `debug-bag-contents` | 1 | HIST-PROSE=1 |
| `generate-descriptions` | 1 | REMOVED-TOOL=1 |
...
- `execution-lifecycle/references/concepts.md:390` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > In MCP tools, the lifecycle is managed through explicit tool calls (`deriva_ml_create_execution`, `deriva_ml_start_execu
- `execution-lifecycle/references/concepts.md:390` **REMOVED-TOOL** — `deriva_ml_create_execution`
  > In MCP tools, the lifecycle is managed through explicit tool calls (`deriva_ml_create_execution`, `deriva_ml_start_execu
...
- `execution-lifecycle/references/concepts.md:493` **HIST-PROSE** — `legacy-marker`
  > > **Known gap:** the legacy `restore_execution` tool has **no equivalent** in the new MCP surface. The replacement patte
- `execution-lifecycle/references/concepts.md:499` **REMOVED-TOOL** — `deriva_ml_create_execution`
  > 3. **Create a fresh execution.** Call `deriva_ml_create_execution(hostname, catalog_id, ...)` with the same workflow, da
- `execution-lifecycle/references/concepts.md:500` **REMOVED-TOOL** — `deriva_ml_start_execution`
  > 4. **Continue the lifecycle as normal.** Start it (`deriva_ml_start_execution`), do the work, and commit (`deriva_ml_com
- `execution-lifecycle/references/concepts.md:500` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > 4. **Continue the lifecycle as normal.** Start it (`deriva_ml_start_execution`), do the work, and commit (`deriva_ml_com
- `execution-lifecycle/references/concepts.md:521` **REMOVED-TOOL** — `deriva_ml_start_execution`
  > All of these can be caught before `deriva_ml_start_execution(...)`.
- `execution-lifecycle/references/concepts.md:527` **HIST-PROSE** — `legacy-marker`
  > | Validate RIDs | `deriva_ml_get_dataset` / `get_entities` | All dataset and asset RIDs exist (legacy `validate_rids` wa
- `execution-lifecycle/references/concepts.md:529` **REMOVED-TOOL** — `deriva_ml_cache_dataset`
  > | Cache data | `deriva_ml_cache_dataset` | Downloads bags/assets into cache without execution provenance |
- `execution-lifecycle/references/concepts.md:539` **REMOVED-TOOL** — `deriva_ml_cache_dataset`
  > | `not_cached` | No local copy | Call `deriva_ml_cache_dataset` if large |
- `execution-lifecycle/references/concepts.md:540` **REMOVED-TOOL** — `deriva_ml_cache_dataset`
  > | `cached_metadata_only` | Table data present, assets not fetched | Call `deriva_ml_cache_dataset(..., materialize=True)
- `execution-lifecycle/references/concepts.md:542` **REMOVED-TOOL** — `deriva_ml_cache_dataset`
  > | `cached_incomplete` | Was cached but assets are missing | Call `deriva_ml_cache_dataset` to re-materialize |
- `execution-lifecycle/references/concepts.md:558` **REMOVED-TOOL** — `deriva_ml_cache_dataset`
...
- `troubleshoot-execution/SKILL.md:147` **REMOVED-TOOL** — `deriva_ml_abort_execution`
  > - **`deriva_ml_abort_execution(hostname, catalog_id, execution_rid, reason="<short explanation>")`** — transitions to `A
- `troubleshoot-execution/SKILL.md:186` **REMOVED-TOOL** — `deriva_ml_start_execution`
  > | `Created` | No | Execution was registered but `start_execution` was never called. No work happened. | Start it (`deriv
- `troubleshoot-execution/SKILL.md:215` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > deriva_ml_commit_execution(hostname=..., catalog_id=..., execution_rid="<rid>", retry_failed=True)
- `troubleshoot-execution/SKILL.md:229` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > deriva_ml_commit_execution(hostname=..., catalog_id=..., execution_rid="<rid>", retry_failed=False)
- `troubleshoot-execution/SKILL.md:245` **REMOVED-TOOL** — `deriva_ml_abort_execution`
  > deriva_ml_abort_execution(hostname=..., catalog_id=..., execution_rid="<bad-rid>",
- `troubleshoot-execution/SKILL.md:249` **REMOVED-TOOL** — `deriva_ml_create_execution`
  > recovery = deriva_ml_create_execution(
- `troubleshoot-execution/SKILL.md:259` **REMOVED-TOOL** — `deriva_ml_start_execution`
  > deriva_ml_start_execution(hostname=..., catalog_id=..., execution_rid=new_rid)
- `troubleshoot-execution/SKILL.md:261` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > deriva_ml_commit_execution(hostname=..., catalog_id=..., execution_rid=new_rid)
- `troubleshoot-execution/SKILL.md:286` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > deriva_ml_commit_execution(hostname=..., catalog_id=..., execution_rid="<bad-rid>")
- `troubleshoot-execution/SKILL.md:294` **REMOVED-TOOL** — `deriva_ml_create_execution`
  > recovery = deriva_ml_create_execution(
...
- `troubleshoot-execution/references/execution-lifecycle.md:287` **HIST-PROSE** — `legacy-marker`
  > Report progress during long-running workflows. The legacy `update_execution_status` was folded into `deriva_ml_update_ex
- `troubleshoot-execution/references/execution-lifecycle.md:292` **HIST-PROSE** — `legacy-marker`
...
- `troubleshoot-execution/references/execution-lifecycle.md:299` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > deriva_ml_commit_execution(hostname="data.example.org", catalog_id="1",
- `troubleshoot-execution/references/execution-lifecycle.md:302` **HIST-PROSE** — `legacy-marker`
  > # Failure marking (replaces the failure path of the legacy stop_execution)
- `troubleshoot-execution/references/execution-lifecycle.md:303` **REMOVED-TOOL** — `deriva_ml_abort_execution`
  > deriva_ml_abort_execution(hostname="data.example.org", catalog_id="1",
- `troubleshoot-execution/references/execution-lifecycle.md:313` **OLD-STATUS-ENUM** — `Status.running`
  > exe.update_status(Status.running, "Loading data...")
- `troubleshoot-execution/references/execution-lifecycle.md:318` **OLD-STATUS-ENUM** — `Status.running`
  > exe.update_status(Status.running, f"Epoch {epoch+1}/100 complete")
- `troubleshoot-execution/references/execution-lifecycle.md:369` **REMOVED-TOOL** — `deriva_ml_commit_execution`
  > - An execution in `Failed`, `Stopped`, or `Pending_Upload` is salvageable — `deriva_ml_commit_execution` (optionally wit
- `troubleshoot-execution/references/execution-lifecycle.md:371` **REMOVED-TOOL** — `deriva_ml_create_execution`
  > - A "recovery execution" is a new execution that consumes the failed run's inputs (Branch C) or its surviving outputs (B
- `troubleshoot-execution/references/execution-lifecycle.md:377` **HIST-PROSE** — `legacy-marker`
...
- `troubleshoot-execution/references/execution-lifecycle.md:437` **HIST-PROSE** — `legacy-marker`
  > Shows row counts and asset sizes per table (the legacy `estimate_bag_size` is subsumed by `deriva_ml_bag_info`). Use to 
- `troubleshoot-execution/references/execution-lifecycle.md:468` **REMOVED-TOOL** — `deriva_ml_update_execution`
  > | `deriva_ml_update_execution(hostname, catalog_id, execution_rid, description="<text>")` | Update an execution's descri
- `troubleshoot-execution/references/execution-lifecycle.md:469` **REMOVED-TOOL** — `deriva_ml_commit_execution`
